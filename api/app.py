    ...
    def download_youtube_content(self, url, path):
        ...
        try:
            ydl_opts = {
                ...
                'format': 'best[height<=720]',  # Lower quality for serverless
                'writesubtitles': True,
        ...
        try:
            ...
            if '/stories/' in url:
                ...
                for post in profile.get_posts():
                    if count >= 5:  # Limit to 5 recent posts for serverless
                        break
...
def bulk_download():
    ...
    try:
        ...
        # Limit bulk downloads for serverless
        if len(urls) > 5:
            return jsonify({'status': 'error', 'message': 'Maximum 5 URLs allowed for bulk download'})
